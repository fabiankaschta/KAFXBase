...
public class ComparatorField<T> extends TextField {
	...
	public ComparatorField(Comparator<T> comparator, ObjectProperty<T> minValue, ObjectProperty<T> maxValue,
			StringConverter<T> converter) {
		this(comparator, minValue, maxValue, converter, false);
	}

	public ComparatorField(Comparator<T> comparator, ObjectProperty<T> minValue, ObjectProperty<T> maxValue,
			StringConverter<T> converter, boolean allowNull) {
		this(comparator, minValue.get(), maxValue.get(), converter);
	...
	public ComparatorField(Comparator<T> comparator, ObjectProperty<T> minValue, ObjectProperty<T> maxValue,
			T initialValue, StringConverter<T> converter) {
		this(comparator, minValue, maxValue, initialValue, converter, false);
	}

	public ComparatorField(Comparator<T> comparator, ObjectProperty<T> minValue, ObjectProperty<T> maxValue,
			T initialValue, StringConverter<T> converter, boolean allowNull) {
		this(comparator, minValue.get(), maxValue.get(), initialValue, converter);
		this.minValue.bind(minValue);
		this.maxValue.bind(maxValue);
		this.allowNull.set(allowNull);
	}

	public ComparatorField(Comparator<T> comparator, T minValue, T maxValue, StringConverter<T> converter) {
		this(comparator, minValue, maxValue, converter, false);
	}

	public ComparatorField(Comparator<T> comparator, T minValue, T maxValue, StringConverter<T> converter,
			boolean allowNull) {
		this(comparator, minValue, maxValue, minValue, converter, allowNull);
	}

	public ComparatorField(Comparator<T> comparator, T minValue, T maxValue, T initialValue,
			StringConverter<T> converter) {
		this(comparator, minValue, maxValue, initialValue, converter, false);
	}
	...
	public ComparatorField(Comparator<T> comparator, T minValue, T maxValue, T initialValue,
			StringConverter<T> converter, boolean allowNull) {
		if (minValue != null && maxValue != null && comparator.compare(minValue, maxValue) > 0) {
			throw new IllegalArgumentException("min value " + minValue + " greater than max value " + maxValue);
		}
		if (initialValue != null && (minValue != null && comparator.compare(minValue, initialValue) > 0
				|| maxValue != null && comparator.compare(maxValue, initialValue) < 0)) {
		...
		value.addListener((_, oldValue, newValue) -> {
			if (newValue == null) {
				setText("");
			} else if (oldValue != null && this.comparator.compare(newValue, oldValue) != 0) {
				if (getMin() != null && this.comparator.compare(newValue, getMin()) < 0) {
					value.setValue(getMin());
					return;
				}
				if (getMax() != null && this.comparator.compare(newValue, getMax()) > 0) {
					value.setValue(getMax());
					return;
				}
				try {
					if (this.comparator.compare(newValue, this.converter.fromString(textProperty().get())) != 0) {
			...
			if (!newValue) {
				...
				if (allowNull() && (text == null || "".equals(text))) {
					...
					try {
						...
						if (getMin() != null && this.comparator.compare(getMin(), newValueGrade) > 0
								|| getMax() != null && this.comparator.compare(newValueGrade, getMax()) > 0) {
		...
		this.textProperty().addListener((_, oldValue, newValue) -> {
			...
			try {
				...
				if (value.get() != null && this.comparator.compare(newValueGrade, value.get()) == 0) {
					return;
				}
